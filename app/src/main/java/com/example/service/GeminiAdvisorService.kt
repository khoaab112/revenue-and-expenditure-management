        ...
        for (attempt in 1..models.size) {
            ...
            try {
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    if (!response.isSuccessful) {
                        Log.e(TAG, "Request failed for model $currentModel: $bodyStr")
                        lastErrorMsg = "Gemini API trả về lỗi (${response.code}) cho model $currentModel"
                        
                        if (response.code == 429) {
                            // Quota exceeded, stop trying other models
                            return@withContext AITransactionResult(
                                success = false,
                                errorMessage = "Vượt quá giới hạn gọi API (Lỗi 429). Vui lòng thử lại sau."
                            )
